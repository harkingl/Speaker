...
public class AudioAnimationView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRect(0, (mTotalHeight-voices.get(0)) / 2, columnWidth, (mTotalHeight+voices.get(0)) / 2, mPaint);
        for (int i = 1; i < voices.size(); i++) {
            float startX = columnWidth*2*i;
            canvas.drawRect(startX, (mTotalHeight-voices.get(i)) / 2, startX + columnWidth,
                    (mTotalHeight+voices.get(i)) / 2, mPaint);
        }
        if (needRunning) {
