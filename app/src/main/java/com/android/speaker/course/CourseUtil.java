package com.android.speaker.course;

public class CourseUtil {
    public static final String KEY_COURSE_ITEM = "course_item";
    public static final String KEY_COURSE_TYPE = "course_type";
    public static final String KEY_SPEAK_DETAIL = "speak_detail";
    public static final String KEY_TITLE = "title";
    public static final String KEY_CHAT_REPORT_INFO = "chat_report_info";
    public static final String KEY_COURSE_ID = "course_id";
    public static final String KEY_COURSE_PREVIEW_INFO = "course_preview_info";

    public static final String KEY_CHAT_ID = "chat_id";

    public static final String KEY_SCENE_ID = "scene_id";

    // 课程类型：0-精品；1-场景 2-专项
    public static final int TYPE_COURSE_CATALOG = 0;
    public static final int TYPE_COURSE_PROJECT = 1;
    public static final int TYPE_COURSE_SPECIAL = 2;
}
